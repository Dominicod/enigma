...
module Encryptable
  ...
  def self.encrypt_message(encrypt_message, key, date)
    ...
    encrypted_message = shift_letter(final_shift, encrypt_message, character_set)
  end
  ...
  def self::shift_letter(final_shift, message, character_set)
    ...
    message.downcase.each_char.with_index do |letter, index|
      ...
      if letter.ord >= 97 && letter.ord <= 122
        ...
        encryption << character_set.rotate(final_shift.values[counter] + set_index).first
        counter += 1
      else
        encryption << letter
        counter += 1
